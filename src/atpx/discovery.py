...
class PysrRegressor:
    ...
    def __init__(self, seed: int, niterations: int, unary: list[str], binary: list[str]) -> None:
        ...
        """
        # pysr ships no stubs and is an optional lane, a genuine third-party gap.
        import pysr  # pyrefly: ignore[missing-import]

        self.version = str(getattr(pysr, "__version__", "unknown"))
        menu = {"unary_operators": unary, "binary_operators": binary}
        ...
        *,
        ...
        try:
            regressor: SymbolicRegressor = PysrRegressor(
                seed, niterations, menu["unary"], menu["binary"]
            )
        except ImportError:
            return self.dormant(data, seed)
        import pandas  # pyrefly: ignore[missing-import]  # noqa: PLC0415
...
def commas(tokens: list[str] | None) -> list[str]:
...
def nmse(predicted: list[float], observed: Iterable[float]) -> float | None:

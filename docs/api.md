...
`ws.sync.verify(...)`, `ws.sync.recall(...)`, `ws.sync.lean(...)`).
...
| `doctor()` | sync | what needs repair, reported and never mutated (plain dict, read only) |
| `settle(zettel, status, message="", judgment=None, counterexample=None, lean=None)` | sync | one evidence-gated status move, returning the journal line |
...
| `log(zettel, who, tag, message)` | sync | one appended journal line, validated to round-trip |
...
`atpx.EvidenceStore`, `atpx.Vault`, `atpx.Zettel`, `atpx.Status`,
